...
extern float car_Dir_out,car_Speed_out;
...
static int set_int_enable(unsigned char enable)
{
    ...
    if (st.chip_cfg.dmp_on) {
        if (enable)
            ...
            tmp = 0x00;
        if (simiic_write_len(st.hw->addr, st.reg->int_enable, 1, &tmp))
            return -1;
...
int mpu_reg_dump(void)
{
    ...
    for (ii = 0; ii < st.hw->num_reg; ii++) {
        if (ii == st.reg->fifo_r_w || ii == st.reg->mem_r_w)
            continue;
        if (simiic_read_len(st.hw->addr, ii, 1, &data))
            return -1;
        (*(printf_t)((*((uint32_t *)((0x1A0) + (uint32)(&(((mydata_in_t *)0)->printf))) ))))(enter_fputc,"%#5x: %#5x\r\n",ii, data);
    }
    return 0;
}
...
int mpu_read_reg(unsigned char reg, unsigned char *data)
{
    ...
    if (reg >= st.hw->num_reg)
        return -1;
    return simiic_read_len(st.hw->addr, reg, 1, data);
}
...
int mpu_init(void)
{
    ...
    data[0] = (0x80);
    if (simiic_write_len(st.hw->addr, st.reg->pwr_mgmt_1, 1, data))
        return -1;
    soft_delay_ms(100);
    ...
    if (simiic_read_len(st.hw->addr, st.reg->accel_offs, 6, data))
        return -1;
    ...
    if (rev) {
        ...
        if (rev == 1)
            ...
            (*(printf_t)((*((uint32_t *)((0x1A0) + (uint32)(&(((mydata_in_t *)0)->printf))) ))))(enter_fputc,"Unsupported software product rev %d.\n",rev);
            return -1;
        }
    } else {
        if (simiic_read_len(st.hw->addr, st.reg->prod_id, 1, data))
            return -1;
        rev = data[0] & 0x0F;
        if (!rev) {
            (*(printf_t)((*((uint32_t *)((0x1A0) + (uint32)(&(((mydata_in_t *)0)->printf))) ))))(enter_fputc,"Product ID read as 0 indicates device is either " "incompatible or an MPU3050.\n");
            return -1;
        } else if (rev == 4) {
            (*(printf_t)((*((uint32_t *)((0x1A0) + (uint32)(&(((mydata_in_t *)0)->printf))) ))))(enter_fputc,"Half sensitivity part found.\n");
            st.chip_cfg.accel_half = 1;
        } else
            st.chip_cfg.accel_half = 0;
    }
    ...
    (*(memset_t)((*((uint32_t *)((0x1A0) + (uint32)(&(((mydata_in_t *)0)->memset))) ))))(&st . chip_cfg . cache, 0, sizeof(st . chip_cfg . cache));
    st.chip_cfg.dmp_on = 0;
    st.chip_cfg.dmp_loaded = 0;
    st.chip_cfg.dmp_sample_rate = 0;
	
	simiic_write_reg((0X68), 0X6B, 0X01);		
...
int mpu_lp_accel_mode(unsigned char rate)
{
    ...
    if (!rate) {
        ...
        if (simiic_write_len(st.hw->addr, st.reg->pwr_mgmt_1, 2, tmp))
            return -1;
        st.chip_cfg.lp_accel_mode = 0;
        return 0;
    }
...
int mpu_get_gyro_reg(short *data, unsigned long *timestamp)
{
    ...
    if (simiic_read_len(st.hw->addr, st.reg->raw_gyro, 6, tmp))
        return -1;
...
int mpu_get_accel_reg(short *data, unsigned long *timestamp)
{
    ...
    if (simiic_read_len(st.hw->addr, st.reg->raw_accel, 6, tmp))
        return -1;
    data[0] = (tmp[0] << 8) | tmp[1];
    data[1] = (tmp[2] << 8) | tmp[3];
    data[2] = (tmp[4] << 8) | tmp[5];
...
int mpu_get_temperature(long *data, unsigned long *timestamp)
{
    ...
    if (simiic_read_len(st.hw->addr, st.reg->temp, 2, tmp))
        return -1;
    raw = (tmp[0] << 8) | tmp[1];
    if (timestamp)
        mget_ms(timestamp);
...
int mpu_set_accel_bias(const long *accel_bias)
{
    ...
    if (simiic_read_len(st.hw->addr, 3, 3, data))
        return -1;
    fg[0] = ((data[0] >> 4) + 8) & 0xf;
    fg[1] = ((data[1] >> 4) + 8) & 0xf;
    fg[2] = ((data[2] >> 4) + 8) & 0xf;
    ...
    if (simiic_read_len(st.hw->addr, 0x06, 6, data))
        return -1;
    ...
    if (simiic_write_len(st.hw->addr, 0x06, 6, data))
        return -1;
    return 0;
}
...
int mpu_reset_fifo(void)
{
    ...
    data = 0;
    if (simiic_write_len(st.hw->addr, st.reg->int_enable, 1, &data))
        return -1;
    if (simiic_write_len(st.hw->addr, st.reg->fifo_en, 1, &data))
        return -1;
    if (simiic_write_len(st.hw->addr, st.reg->user_ctrl, 1, &data))
        return -1;
    ...
    if (st.chip_cfg.dmp_on) {
        ...
        if (simiic_write_len(st.hw->addr, st.reg->int_enable, 1, &data))
            return -1;
        if (simiic_write_len(st.hw->addr, st.reg->fifo_en, 1, &st.chip_cfg.fifo_enable))
            return -1;
    }
    return 0;
}
...
int mpu_set_gyro_fsr(unsigned short fsr)
{
    ...
    if (st.chip_cfg.gyro_fsr == (data >> 3))
        return 0;
    if (simiic_write_len(st.hw->addr, st.reg->gyro_cfg, 1, &data))
        return -1;
...
int mpu_set_accel_fsr(unsigned char fsr)
{
    ...
    if (st.chip_cfg.accel_fsr == (data >> 3))
        return 0;
    if (simiic_write_len(st.hw->addr, st.reg->accel_cfg, 1, &data))
        return -1;
...
int mpu_set_lpf(unsigned short lpf)
{
    ...
    if (st.chip_cfg.lpf == data)
        return 0;
    if (simiic_write_len(st.hw->addr, st.reg->lpf, 1, &data))
        return -1;
...
int mpu_set_sample_rate(unsigned short rate)
{
    ...
    if (st.chip_cfg.dmp_on)
        ...
        data = 1000 / rate - 1;
        if (simiic_write_len(st.hw->addr, st.reg->rate_div, 1, &data))
            return -1;
...
int mpu_set_sensors(unsigned char sensors)
{
    ...
    if (!(sensors & (0x08)))
        data |= ((0x20) | (0x10) | (0x08));
    if (simiic_write_len(st.hw->addr, st.reg->pwr_mgmt_2, 1, &data)) {
        st.chip_cfg.sensors = 0;
        return -1;
    }
...
int mpu_get_int_status(short *status)
{
    ...
    if (!st.chip_cfg.sensors)
        return -1;
    if (simiic_read_len(st.hw->addr, st.reg->dmp_int_status, 2, tmp))
        return -1;
        ...
        unsigned char *sensors, unsigned char *more)
{
    ...
    if (simiic_read_len(st.hw->addr, st.reg->fifo_count_h, 2, data))
        return -1;
    ...
    if (fifo_count > (st.hw->max_fifo >> 1)) {
        ...
        if (simiic_read_len(st.hw->addr, st.reg->int_status, 1, data))
            return -1;
        if (data[0] & (0x10)) {
            mpu_reset_fifo();
            return -2;
    ...
    mget_ms((unsigned long*)timestamp);

    if (simiic_read_len(st.hw->addr, st.reg->fifo_r_w, packet_size, data))
        return -1;
    ...
    unsigned char *more)
{
    ...
    if (simiic_read_len(st.hw->addr, st.reg->fifo_count_h, 2, tmp))
        return -1;
    ...
    if (fifo_count > (st.hw->max_fifo >> 1)) {
        ...
        if (simiic_read_len(st.hw->addr, st.reg->int_status, 1, tmp))
            return -1;
        if (tmp[0] & (0x10)) {
            mpu_reset_fifo();
            return -2;
    ...
    if (simiic_read_len(st.hw->addr, st.reg->fifo_r_w, length, data))
        return -1;
...
int mpu_set_bypass(unsigned char bypass_on)
{
    ...
    if (bypass_on) {
        if (simiic_read_len(st.hw->addr, st.reg->user_ctrl, 1, &tmp))
            return -1;
        tmp &= ~(0x20);
        if (simiic_write_len(st.hw->addr, st.reg->user_ctrl, 1, &tmp))
            return -1;
        soft_delay_ms(3);
        tmp = (0x02);
        ...
        if (st.chip_cfg.latched_int)
            tmp |= (0x20) | (0x10);
        if (simiic_write_len(st.hw->addr, st.reg->int_pin_cfg, 1, &tmp))
            return -1;
    } else {
...
static int get_accel_prod_shift(float *st_shift)
{
    ...
    if (simiic_read_len(st.hw->addr, 0x0D, 4, tmp))
        return 0x07;
...
static int gyro_self_test(long *bias_regular, long *bias_st)
{
    ...
    if (simiic_read_len(st.hw->addr, 0x0D, 3, tmp))
        return 0x07;
...
static int get_st_biases(long *gyro, long *accel, unsigned char hw_test)
{
    ...
    data[0] = 0x01;
    data[1] = 0;
    if (simiic_write_len(st.hw->addr, st.reg->pwr_mgmt_1, 2, data))
        return -1;
    soft_delay_ms(200);
    data[0] = 0;
    if (simiic_write_len(st.hw->addr, st.reg->int_enable, 1, data))
    ...
    if (simiic_write_len(st.hw->addr, st.reg->pwr_mgmt_1, 1, data))
        return -1;
    if (simiic_write_len(st.hw->addr, st.reg->i2c_mst, 1, data))
        return -1;
    if (simiic_write_len(st.hw->addr, st.reg->user_ctrl, 1, data))
        return -1;
    data[0] = (0x04) | (0x08);
    ...
    for (ii = 0; ii < packet_count; ii++) {
        ...
        if (simiic_read_len(st.hw->addr, st.reg->fifo_r_w, (12), data))
            return -1;
        ...
        unsigned char *data)
{
    ...
    if (simiic_write_len(st.hw->addr, st.reg->bank_sel, 2, tmp))
        return -1;
    if (simiic_write_len(st.hw->addr, st.reg->mem_r_w, length, data))
        return -1;
    return 0;
}
        ...
        unsigned char *data)
{
    ...
    if (simiic_write_len(st.hw->addr, st.reg->bank_sel, 2, tmp))
        return -1;
    if (simiic_read_len(st.hw->addr, st.reg->mem_r_w, length, data))
        return -1;
    return 0;
}
    ...
    unsigned short start_addr, unsigned short sample_rate)
{
    ...
    if (simiic_write_len(st.hw->addr, st.reg->prgm_start_h, 2, tmp))
        return -1;
...
int mpu_set_dmp_state(unsigned char enable)
{
    ...
    if (enable) {
        ...
        tmp = 0;
        simiic_write_len(st.hw->addr, 0x23, 1, &tmp);
    ...
    unsigned char lpa_freq)
{
    ...
    if (lpa_freq) {
        ...
        data[0] = thresh_hw;
        data[1] = time;
        if (simiic_write_len(st.hw->addr, st.reg->motion_thr, 2, data))
            goto lp_int_restore;

         
        soft_delay_ms(5);
        data[0] = (st.chip_cfg.accel_fsr << 3) | (0x07);
        if (simiic_write_len(st.hw->addr, st.reg->accel_cfg, 1, data))
            goto lp_int_restore;
        ...
        if (lpa_freq == 1)
            ...
            data[1] = INV_LPA_40HZ;
        data[1] = (data[1] << 6) | ((0x04) | (0x02) | (0x01));
        if (simiic_write_len(st.hw->addr, st.reg->pwr_mgmt_1, 2, data))
            goto lp_int_restore;
...
uint8 mpu_dmp_init(void)
{
	...
	if(mpu_init()==0)	
	{	 
		res=mpu_set_sensors(((0x40) | (0x20) | (0x10))|(0x08));
		if(res)return 1; 
		res=mpu_configure_fifo(((0x40) | (0x20) | (0x10))|(0x08));
		if(res)return 2; 
		res=mpu_set_sample_rate(1000);	
		if(res)return 3; 
