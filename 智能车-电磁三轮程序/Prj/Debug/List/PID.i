...
{ 
    ...
    d_d_PID.LastError = 0; 
    d_d_PID.PrevError = 0; 
    d_d_PID.Proportion =0; 
    d_d_PID.Integral =0; 
    d_d_PID.Derivative =0; 
    d_d_PID.SetPoint = 0;
...
float Speed_PID(int Speed_L,int Speed_R)
{
    register float iError, Speed_out,Speed;    
    Speed = (float)((Speed_L+Speed_R)/2.0);
    iError = s_PID.SetPoint - Speed;   
...
float dir_PID()
{
  ...
  d_PID.SumError += iError; 
  dError = iError - d_PID.LastError; 
...
float dir_R_PID()
{
  register float iError,Dir_out,dError;
  iError = dianci_manage();
  R_PID.SumError += iError; 
  dError = iError - R_PID.LastError; 
...
float dir_d_PID(float NowData, float Point)
{
  ...
  d_d_PID.SumError += iError; 
  dError = iError - d_d_PID.LastError; 
