...
{ 
 ...
 static PID s_PID,d_PID,d_d_PID;
...
{ 
	...
	simiic_write_reg((0X68), 0X6B, 0X80);		
  	soft_delay_ms(10);
	simiic_write_reg((0X68), 0X6B, 0X00);		
	soft_delay_ms(10);
	simiic_write_reg((0X68), 0X6B, 0X01);		
	soft_delay_ms(10);
	simiic_write_reg((0X68), 0X1B, 3<<3);		
	soft_delay_ms(10);
	simiic_write_reg((0X68), 0X1C, 0<<3);		
	soft_delay_ms(10);
	simiic_write_reg((0X68), 0X19, 0X00);	
	soft_delay_ms(10);
	simiic_write_reg((0X68), 0X1A, 0X01);			
	soft_delay_ms(100);
	
	res=simiic_read_reg((0X68), 0X75, IIC);
	if(res==(0X68))
	{
...
void MPU6050_GetData(S_INT16_XYZ *GYRO, S_INT16_XYZ *ACC)
{
	if (Offset_OK)
	{
		...
		GYRO->X = GetData(0X43) - GYRO_Offset.X;	
		GYRO->Y = GetData(0X45) - GYRO_Offset.Y;
		GYRO->Z = GetData(0X47) - GYRO_Offset.Z;
	}
	else
	{
		ACC->X = GetData(0X3B);	
...
int16 GetData(uchar REG_Address)
{
	...
	H = simiic_read_reg((0X68), REG_Address, IIC);
	L = simiic_read_reg((0X68), REG_Address+1, IIC);
	
	return ((H<<8)|L);   
}


int32 ACC_X_BUF[5], ACC_Y_BUF[5], ACC_Z_BUF[5];	
int32 GYRO_X_BUF[6], GYRO_Y_BUF[6], GYRO_Z_BUF[6];
...
void Data_Filter(void)	
{
	...
	ACC_Real.X = temp1 / 5 / 16384.0f;
	ACC_Real.Y = temp2 / 5 / 16384.0f;
	ACC_Real.Z = temp3 / 5 / 16384.0f;
	GYRO_Real.X = temp4 / 6 / 16.4f;
	GYRO_Real.Y = temp5 / 6 / 16.4f;
	GYRO_Real.Z = temp6 / 6 / 16.4f;
	...
	for(i = 0; i < 6 - 1; i++)
	{
		GYRO_X_BUF[6-1-i] = GYRO_X_BUF[6-2-i];
		GYRO_Y_BUF[6-1-i] = GYRO_Y_BUF[6-2-i];
		GYRO_Z_BUF[6-1-i] = GYRO_Z_BUF[6-2-i];
	}
}
...
void KalmanFilter(float ACC_Angle)
{
    ...
    Priori_Estimation = Posterior_Estimation - GYRO_Real.Y*1/500.0f;		
    ...
    Priori_Convariance = (float)sqrt( Posterior_Convariance * Posterior_Convariance + 2.0f * 2.0f );
    ...
    KalmanGain = (float)sqrt( Priori_Convariance * Priori_Convariance / ( Priori_Convariance * Priori_Convariance + 5000.0f * 5000.0f ) );
...
void Get_Attitude(void)	
{
	IMUupdate(GYRO_Real.X*0.0010653*16.4f, 
			  GYRO_Real.Y*0.0010653*16.4f, 
			  GYRO_Real.Z*0.0010653*16.4f, 
			  ACC_Real.X * 16384.0f, 
			  ACC_Real.Y * 16384.0f, 
			  ACC_Real.Z * 16384.0f);	
}
...
void IMUupdate(float gx, float gy, float gz, float ax, float ay, float az)
{
	...
	exInt = exInt + ex * 0.001f;	
	eyInt = eyInt + ey * 0.001f;
	ezInt = ezInt + ez * 0.001f;
	...
	q0 = q0 + (-q1*gx - q2*gy - q3*gz)*0.005f;
	q1 = q1 + (q0*gx + q2*gz - q3*gy)*0.005f;
	q2 = q2 + (q0*gy - q1*gz + q3*gx)*0.005f;
	q3 = q3 + (q0*gz + q1*gy - q2*gx)*0.005f;
	...
	Attitude_Angle.Y = asin(-2*q1*q3 + 2*q0*q2) * 57.3; 
