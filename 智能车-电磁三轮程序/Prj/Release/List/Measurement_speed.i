...
void Measurement_speed(void)
{
  ...
  int16_t speed_left,speed_right; 
    ...
    ftm_pulse_mux(((FTM_Type *)(0x40039000u)), PTE0);
    ftm_clean(((FTM_Type *)(0x40039000u))) ;
    soft_delay_ms(5);
    speed_left = ftm_pulse_get(((FTM_Type *)(0x40039000u)));
    
    
    
    ftm_pulse_mux(((FTM_Type *)(0x40039000u)), PTE7);
    ftm_clean(((FTM_Type *)(0x40039000u))) ;
    soft_delay_ms(5);
    speed_right = ftm_pulse_get(((FTM_Type *)(0x40039000u)));
    
    for(i=0;i<9;i++)
    {
      count_left_l[i]=count_left_l[i+1];
    }
    for(i=0;i<9;i++)
    {
      count_right_r[i]=count_right_r[i+1];
    }
    count_left_l[9]=speed_left;
    count_right_r[9]=speed_right;
    
    for(i=0;i<9;i++)
    {
      sum_l+=count_left_l[i];
    }
    for(i=0;i<9;i++)
    {
      sum_r+=count_right_r[i];
    }
    if((((Dtype *)(&(GPIOX[1]->PDIR )))->b14)==0)
    {
      count_right = -sum_r/10;
    }
    else
    {
      count_right=sum_r/10;
